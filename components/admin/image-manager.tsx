...
import { Check, User, Laptop, ImageIcon, FileImage, Copy } from "lucide-react"
...
// Reemplazar la constante iconRepositories por imageServices
const imageServices = [
  {
    name: "Cloudinary",
...
export default function ImageManager() {
  ...
  // Añadir esta función para copiar al portapapeles
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="space-y-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        {/* Contenido para cada tipo de imagen */}
        {imageTypes.map((type) => (
          <TabsContent key={type.id} value={type.id} className="mt-0 space-y-6">
            <Card className="bg-black/40 border-blue-700/20">
              <CardHeader>
                <CardTitle className="flex items-center">
                  {type.icon}
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <ul className="space-y-2 text-sm text-slate-300">
                      <li className="flex items-start">
                        ...
                        <span>Dimensiones: {type.recommendations.dimensions}</span>
                      </li>
                      <li className="flex items-start">
                        <Check className="h-4 w-4 text-blue-500 mr-2 mt-0.5" />
                        <span>Formato: {type.recommendations.format}</span>
                      </li>
                      <li className="flex items-start">
                        <Check className="h-4 w-4 text-blue-500 mr-2 mt-0.5" />
                        <span>Tamaño de archivo: {type.recommendations.fileSize}</span>
                      </li>
                      <li className="flex items-start">
                        <Check className="h-4 w-4 text-blue-500 mr-2 mt-0.5" />
                        <span>
                          Ubicación recomendada: <code>{type.path}</code>
                        </span>
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-black/60 p-3 rounded-md text-xs font-mono text-slate-300">
                      {type.recommendations.examples.map((example, index) => (
                ...
                {/* Imágenes actuales según el tipo */}
                {type.id === "profile" && (
                  <div className="mt-6">
      ...
      {/* Sección de servicios para imágenes */}
      <Card className="bg-black/40 border-blue-700/20">
        <CardHeader>
